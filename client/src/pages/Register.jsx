...
export default function Register() {
  ...
  const perks = ['AI-powered resume generation', 'ATS-optimized templates', 'One-click PDF export']
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-50 to-accent-50 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex items-center gap-2 font-bold text-2xl text-primary-700">
            <FileText className="w-7 h-7" /> ResumeAI
          </Link>
          <h1 className="text-2xl font-bold text-gray-900 mt-4">Create your free account</h1>
          <p className="text-gray-500 text-sm mt-1">Start building AI-powered resumes today</p>
        </div>

        {/* Perks */}
        <div className="bg-primary-50 border border-primary-100 rounded-xl p-4 mb-5 space-y-2">
          {perks.map((p) => (
            <div key={p} className="flex items-center gap-2 text-sm text-primary-700">
              <CheckCircle className="w-4 h-4 text-primary-500" /> {p}
            </div>
          ))}
        </div>
        ...
        <div className="card shadow-lg">
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 border border-gray-200 rounded-lg py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-all mb-5 disabled:opacity-60"
          >
